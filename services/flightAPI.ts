...
if (!RAPIDAPI_KEY) {
  throw new Error(
    "EXPO_PUBLIC_RAPIDAPI_KEY is not defined in environment variables. Please check your .env file and ensure the variable name starts with EXPO_PUBLIC_."
  );
}
...
export const flightAPI = {
  ...
  searchAirports: async (params: AirportSearchParams): Promise<Airport[]> => {
    try {
      const response: AxiosResponse<ApiResponse<Airport[]>> = await api.get(
  ...
  searchFlights: async (
    ...
    try {
      ...
      if (!response.data.status) {
        throw new Error(
          `Failed to search flights - API returned status false. Response: ${JSON.stringify(
            response.data
          )}`
        );
      }
